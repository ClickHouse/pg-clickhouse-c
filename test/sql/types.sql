...
-- Apply type modifiers through cast functions
SELECT pgch_decode_as(pgch_encode('Int32', 5::int4), NULL::bit(8)),
       pgch_decode_as(pgch_encode('Int64', 5::int8), NULL::bit(12)),
       pgch_decode_as(pgch_encode('Array(Int32)', ARRAY[1, 2]::int4[]),
                      NULL::bit(4)[]);

-- Encode without a type modifier and request an explicit cast
CREATE TYPE csuit AS ENUM ('hearts', 'spades');
CREATE FUNCTION csuit_text(csuit, int4, bool) RETURNS text LANGUAGE sql IMMUTABLE AS
    $$ SELECT (CASE $1 WHEN 'hearts' THEN 'h' ELSE 's' END) || $2 || $3 $$;
CREATE CAST (csuit AS text) WITH FUNCTION csuit_text(csuit, int4, bool);
SELECT pgch_roundtrip('String', 'spades'::csuit);
